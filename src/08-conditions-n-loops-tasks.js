...
function doRectanglesOverlap(rect1, rect2) {
  const cond1 = rect1.height >= rect2.left;
  const cond2 = rect1.width >= rect2.top;
  return cond1 && cond2;
}
...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (arr.indexOf(arr[i]) === i && str.indexOf(arr[i], i + 1) === -1) {
      return arr[i];
    }
  }
  return null;
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const frtsBracket = isStartIncluded ? '[' : '(';
  const lstBracket = isEndIncluded ? ']' : ')';
  const sortedStr = (a > b) ? `${b}, ${a}` : `${a}, ${b}`;
  return frtsBracket + sortedStr + lstBracket;
}
...
function getDigitalRoot(num) {
  ...
  while (Math.floor(num / 10)) {
    ...
    summ = array.reduce((previousValue, currentValue) => +previousValue + +currentValue);
